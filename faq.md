...
1. **本地文件通道（推荐，零风险）**：外部进程用任意环境（py3.12、akshare、自建库均可）取数→落地 csv/json→内置策略只读本地文件。白名单管不到、单线程不阻塞、外部进程崩了策略只是用旧数据不会挂。`MiniQMT-demo` 的 URL 票池模式即转换为此架构（examples.md 第8节）。
...
3. **QMT 自身数据**：内置数据接口反而比 mini 的 `xtdata` 更全——财务数据、龙虎榜、北向资金、ETF申赎清单、L2（需权限）都有（`docs/qmt_api/innerApi/data_function.md`）。原策略从外部源取的数据，先查内置接口是否已覆盖。

**推荐架构**："外算内执行"——重数据、重计算留在外部进程，内置端只读结果文件并执行交易。这正是文件桥方案的单向简化版。
...
| 驱动源 | 最短间隔 | 说明 |
|---|---|---|
| `C.run_time("f", "5nSecond", ...)` | **毫秒级**（`"500nMilliSecond"`） | 与主图 K 线周期完全无关——周期选日线照样每秒跑（本仓库生产桥即 `1nSecond` 循环，实盘长期稳定） |
...
- 每策略每轮回调耗时控制在 <100ms。`get_trade_detail_data`/`get_full_tick` 读本地内存缓存，毫秒级，每秒调用无压力（生产桥实测每秒全套查询+文件IO稳定）。
